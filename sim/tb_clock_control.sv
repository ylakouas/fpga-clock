...
// always forces back to RUN, sec_clear fires leaving SET_MIN -> RUN
...
module tb_clock_control;
    ...
    initial begin
        ...
        if (run_en === 1'b1)
            $display("pass starts in RUN");
        else
            $display("FAIL did not start in RUN");
        ...
        if (run_en === 1'b0)
            $display("pass left RUN after mode press");
        else
            $display("FAIL still in RUN after mode press");
        ...
        if (run_en === 1'b1)
            $display("pass back in RUN after full mode cycle");
        else
            $display("FAIL did not return to RUN");
        ...
        if (run_en === 1'b1)
            $display("pass reset forced state back to RUN");
        else
            $display("FAIL reset did not return to RUN");
